...
\parencite{plant2016} needed to configure the device and to record from
it: the capture command, the input-check and event-marking commands, and
the commands that read and write sensor thresholds and the smoothing
mask. It is, in its present state, an \emph{input-side} tool. Although
the BBTK is also a signal generator, able to drive sounders, TTL outputs
and actuator closures, none of the corresponding commands are currently
implemented, and the suite cannot be used to generate stimuli. Users who
need the BBTK to act as a source rather than an observer must fall back
on the vendor's software.
...
20~ms, which must be subtracted before interpretation.
