...
\abstract{%
  ...
  measurements. One limitation is that the software provided along the
  BBTK by the vendor is specific for Windows. We developped a
  cross-plateform suite of command line tools and a library designed
  to interface with the BBTK v3. These tools included serial port
...
\begin{figure*}[ht]
    ...
    \caption{Principle of operation: The BBTK acts as an independent
      observer of the stimulation device's outputs. Here a optical
      sensor detects when the illumination of the screen reaches a
      certain threshold. Here, the `bbtk-capture` program that drives
...
\texttt{bbtkv3} software suite that we developped encapsulates this
...
OCThe primary uses of \texttt{bbtkv3} is the objective measurement of
...
4 keypad inputs). All the user has to do is enter the command shown on Listing~1 below.
...
\texttt{.events.csv} file consists of a a table with three columns:
\texttt{type, onset, duration}, where \texttt{type} indicates the
sensor (\texttt{Opto1}, \texttt{Mic1}, \texttt{TTLin1}, ...
...
\texttt{events-stats}, which can immediatly outputs descriptive
statistics and ASCII histograms about the timing data. It computes a)
...
photodiode detection). Figure~\ref{stats} shows an example of the
output from a simple stimulation program (1 frame on --- 2 frames off)
ran on a Raspberry 4.
